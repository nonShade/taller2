...
class FaceCropper:
    ...
    def detect_faces(self, image_path: str) -> List[Tuple[np.ndarray, float]]:
        ...
        try:
            ...
            # Detectar y extraer rostros directamente
            ...
            faces = []
            # El MTCNN ya devuelve rostros recortados y normalizados
            if face_tensor.dim() == 4:  # Múltiples rostros
                for face in face_tensor:
                    # Convertir de tensor [-1, 1] a numpy [0, 255]
                    face_array = face.permute(1, 2, 0).numpy()
                    face_array = ((face_array + 1) * 127.5).astype(np.uint8)
                    # Confianza fija ya que pasó el filtro
                    faces.append((face_array, 0.95))
            else:  # Un solo rostro
    ...
    def crop_faces(self, input_dir: str, output_dir: str) -> dict:
        ...
        if not os.path.exists(input_dir):
            raise FileNotFoundError(
                f"Directorio de entrada no encontrado: {input_dir}")
            ...
            try:
                ...
                if faces:
                    ...
                    for i, (face, confidence) in enumerate(faces):
                        ...
                        print(f"  └─ Rostro guardado: {
                              output_filename} (confianza: {confidence:.3f})")
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Detector y recortador de rostros')
